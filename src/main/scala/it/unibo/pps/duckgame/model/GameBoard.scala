...
import it.unibo.pps.duckgame.model.cell.specialCell.{
  SpecialCell,
  SpecialCellType
}
...
/** Represents the game board
  ...
  *   A list of cells
  * @param _specialCells
  *   A list of SpecialCell objects representing special cells with unique
  *   effects on gameplay. These cells might provide bonuses, penalties, or
  *   trigger events when a player lands on them.
  */
class GameBoard(
  cells: List[Cell],
  ...
  /** Gets a reference to the list of special cells on the game board.
    *
    * @return
    *   An immutable list of `SpecialCell` objects on the game board. Modifying
    *   this list will not affect the internal game state.
    */
...
object GameBoard:
  def apply(): GameBoard =
    new GameBoard(
      ...
      SetupFromFile.setup[SpecialCell](TxtResources.SPECIAL_CELL_TXT.path)(
        SpecialCellsParser
      )
    )
