...
class CellTest extends AnyFlatSpec with should.Matchers:
  ...
  private def TOTAL_CELLS = 64

  "Skeleton's special cell in the gameboard cells's list" should "have index 58" in {
    GameReader.gameBoard.specialCells.find(cell =>
      cell.number == SKELETON_CELL
    ) shouldBe defined
  }
  ...
  "GameBoard special cells list" should "contain correct cells' types" in {
    GameReader.gameBoard.specialCells
      .find(_.number == FINAL_CELL)
      .orNull
      .specialCellType shouldBe SpecialCellType.FINAL
  }

  "GameBoard's number of cells" should "be 64" in {
    GameReader.gameBoard.size shouldBe TOTAL_CELLS
  }
